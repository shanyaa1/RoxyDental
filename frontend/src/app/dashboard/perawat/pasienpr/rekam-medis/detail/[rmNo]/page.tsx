...
export default function RekamMedisDetailPage({ params }: DetailProps) {
  const [isDownloading, setIsDownloading] = useState(false);

  const rm = params.rmNo.trim();
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      if (record?.examination?.note) {
        ...
        let ty = cursorY + 10;
        for (const ln of splitTextToLines(
          `Catatan: ${record.examination.note}`,
          font,
          FONT_SIZE_NORMAL,
          CONTENT_WIDTH - 20
        )) {
      ...
      if (!Array.isArray(meds) || meds.length === 0) {
        drawTable(
          [["Obat", "Tidak ada obat yang diresepkan."]],
          [90, CONTENT_WIDTH - 90]
        );
      } else {
        const medRows = meds.map((m: any, i: number) => [
          (i + 1).toString(),
          `${m.name}${m.dosage ? " | Dosis: " + m.dosage : ""}${
            m.duration ? " | Durasi: " + m.duration : ""
          }${m.instructions ? " | Petunjuk: " + m.instructions : ""}`,
        ]);
      ...
      (pdfDoc.getPages() as any[]).forEach((p: any, idx: number) => {
        ...
        p.drawText(`RoxyDental ${new Date().getFullYear()}`, {
          x: MARGIN,
          y: FOOTER_HEIGHT - 34,
          size: 9,
          font,
            ...
            <Card className="shadow-lg">
              ...
              <CardContent className="space-y-4 bg-white text-sm pt-4">
                <div>
                  ...
                  <p className="font-medium mt-1">
                    {record.examination?.chiefComplaint ?? "-"}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium mt-1">
                    {record.examination?.treatmentPlan ?? "-"}
                  </p>
                </div>
              ...
              <CardContent className="space-y-3 bg-white text-sm pt-4">
                {meds.length === 0 ? (
                  ...
                  meds.map((m, idx) => (
                    <div
                      key={m.id ?? idx}
                      className="rounded-md bg-pink-50 p-4 border border-pink-100"
                    >
